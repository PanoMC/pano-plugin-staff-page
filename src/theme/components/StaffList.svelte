    ...
    <div class="staff-list">
      {#each staff as member}
        <div class="staff-item-list d-flex align-items-center p-3 mb-3 border rounded">
          <img src={member.avatarUrl || 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + member.name} alt={member.name} class="rounded-circle me-4" style="width: 80px; height: 80px; object-fit: cover;" />
          <div class="flex-grow-1">
            <h4 class="mb-1">{member.name}</h4>
            <span class="badge bg-primary-subtle text-primary mb-2">{member.role}</span>
            <p class="mb-0 text-muted small">{member.description || ''}</p>
          </div>
          <div class="socials d-flex gap-2">
            {#each parseSocial(member.socialLinks) as [platform, url]}
              <a href={url.startsWith('http') ? url : '#'} target="_blank" class="btn btn-sm btn-outline-secondary" title={platform} aria-label={platform}>
        ...
        <div class="col-md-6 col-lg-4">
          <div class="staff-card text-center h-100 p-4 border rounded shadow-sm">
            <div class="staff-avatar-wrapper mb-3 position-relative d-inline-block">
               <img src={member.avatarUrl || 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + member.name} alt={member.name} class="rounded-circle" style="width: 120px; height: 120px; object-fit: cover;" />
            </div>
            <h4 class="mb-1">{member.name}</h4>
            ...
            <div class="socials d-flex justify-content-center gap-2 mt-auto">
              {#each parseSocial(member.socialLinks) as [platform, url]}
                <a href={url.startsWith('http') ? url : '#'} target="_blank" class="social-link" title={platform} aria-label={platform}>
                  <i class={getSocialIcon(platform)}></i>
                </a>
              {/each}
            </div>
    ...
    <div class="row g-3">
      {#each staff as member}
        <div class="col-6 col-md-4 col-lg-3">
          <div class="staff-grid-item position-relative overflow-hidden rounded ratio ratio-1x1 border">
            <img src={member.avatarUrl || 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + member.name} alt={member.name} class="w-100 h-100 object-fit-cover" />
            <div class="staff-overlay p-3 d-flex flex-column justify-content-end text-white">
              ...
              <div class="socials d-flex gap-2 mt-2">
                {#each parseSocial(member.socialLinks) as [platform, url]}
                  <a href={url.startsWith('http') ? url : '#'} target="_blank" class="text-white opacity-75 hover-opacity-100" aria-label={platform}>
                    <i class={getSocialIcon(platform)}></i>
                  </a>
                {/each}
              </div>
