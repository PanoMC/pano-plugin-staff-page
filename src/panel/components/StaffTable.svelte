  ...
  <table class="table table-hover">
    <thead>
      <tr>
        ...
        <th scope="col" class="align-middle text-center">{$_('staff.role')}</th>
        ...
        <tr>
          <td colspan="5" class="text-center py-5">
            <div class="spinner-border text-primary" role="status">
              <span class="visually-hidden">{$_('common.loading') || 'Loading...'}</span>
            </div>
          </td>
        </tr>
      {:else}
        {#each staff as member (member.id)}
          <StaffRow {member} onEdit={handleEdit} onDelete={handleDelete} />
        {/each}
      {/if}
    </tbody>
  </table>
